...
async def async_read(executor, f_path, chunk_size, queue):
    ...
    with open(f_path, 'r') as fd:
        while True:
            ...
            data = await loop.run_in_executor(executor, long_time_task,
                                              fd, chunk_size)
...
async def main(executor):
    ...
    print("Type of 'consumer = asyncio.ensure_future(async_consume(queue))':"
          "{}".format(type(consumer)))
...
if __name__ == "__main__":
    ...
    # We close the loop, from now on it is not usable
    # but we need to create a new one.
    loop.close()
